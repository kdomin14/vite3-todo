<template>
  <div class="min-h-screen flex flex-col justify-center items-center">
    ...
    <h3 class="text-3xl font-thin text-green-800">Pending Items : 
      {{ pendingTodos.length }}
    </h3>
    <ul class = "space-y-4">
      <li 
      v-for= "todo in pendingTodos" :key="todo.id"
      class="bg-white rounded-xl text-2xl py-2 px-4 font-light text-green-800 text-center hover:text-white hover:bg-green-800 cursor-pointer transition-colors"
